...
try {
    ...
    $stmt = $pdo->prepare("
    SELECT COUNT(*) AS duplicateCount, e.election_name
    FROM parties p
    INNER JOIN elections e ON p.election_id = e.id
    WHERE p.name = ? AND p.election_id = ?
");

    $stmt->execute([$name, $election]);
    $result = $stmt->fetch(PDO::FETCH_ASSOC);

    $duplicateCount = $result['duplicateCount'];
    $electionName = $result['election_name'] ?? 'Unknown Election';

    if ($duplicateCount > 0) {
        echo json_encode(['status' => 'error', 'message' => "A party with the name '$name' already exists in the '$electionName' election. Please choose a different party name."]);
        exit;
    }
    ...
    $stmt = $pdo->prepare("
        INSERT INTO parties (name, election_id, platforms, party_image, status) 
        VALUES (:name, :election_id, :platforms, :party_image, :status)
    ");
    $stmt->execute([
        ':name' => $name,
