...
// Must be a POST request to create a new academic year
...
$custom_voter_option = isset($_POST['custom_voter_option']) ? 1 : 0;

if (!$semester || !$start_year_input || !$start_date || !$end_date) {
...
// ── Build new school-year label e.g. "2025 - 2026" ─────────────────────────
$start_year  = intval($start_year_input);
$end_year    = $start_year + 1;
$school_year = "$start_year - $end_year";

// ============================================================
// STEP 1 — Archive everything that belongs to the current
//          Ongoing academic year, for every election/voting
//          period that lives under it.
// ============================================================
$stmt = $pdo->prepare("SELECT id FROM academic_years WHERE status = 'Ongoing' LIMIT 1");
$stmt->execute();
$ongoingAcademicYearId = $stmt->fetchColumn();

if ($ongoingAcademicYearId) {

    // Fetch ALL voting periods tied to the ongoing academic year
    $stmt = $pdo->prepare("
        SELECT vp.id AS voting_period_id, vp.election_id, e.election_name
        FROM voting_periods vp
        JOIN elections e ON vp.election_id = e.id
        WHERE e.academic_year_id = ?
    ");
    $stmt->execute([$ongoingAcademicYearId]);
    $votingPeriods = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if ($votingPeriods) {
        foreach ($votingPeriods as $vpRow) {
            $voting_period_id = $vpRow['voting_period_id'];
            $election_id      = (int)$vpRow['election_id'];
            ...
            // ── Archive academic year reference data ──────────────────────
            // BUG FIX (Bug 4): Pass $academic_year_id so only this year is archived,
            // not every non-archived academic year in the system.
            archiveAcademicYear($pdo, $archivePdo, $academic_year_id);
            $ayDataNew = fetchAcademicYearData($pdo, $academic_year_id);
            archiveAcademicInfo($archivePdo, $ayDataNew);

            // ── Archive election-specific data ────────────────────────────
            archiveVotingPeriod($archivePdo, $vpData);
            archiveElection($pdo, $archivePdo, $election_id, $voting_period_id);
            archivePositions($pdo, $archivePdo, $election_id);
            archiveVotes($pdo, $archivePdo, $voting_period_id);
            archiveCandidacyData($pdo, $archivePdo, $election_id, $voting_period_id);
            archiveParties($pdo, $archivePdo, $election_id, $voting_period_id);

            // ── Archive with count tracking ───────────────────────────────
            $archivedCounts = ['precincts' => 0, 'voters' => 0, 'candidates' => 0];

            $candidateMsg = archiveCandidates($pdo, $archivePdo, $election_id, $voting_period_id);
            preg_match('/\d+/', $candidateMsg, $matches);
            $archivedCounts['candidates'] = (int)($matches[0] ?? 0);

            // BUG FIX (Bug 1): archiveVoters MUST run before archivePrecincts.
            // archivePrecincts deletes precinct_voters rows that archiveVoters
            // depends on for precinct_name, precinct_type, and has_voted.
            $voterMsg = archiveVoters($pdo, $archivePdo, $election_id, $voting_period_id);
            preg_match('/\d+/', $voterMsg, $matches);
            $archivedCounts['voters'] = (int)($matches[0] ?? 0);

            // Precincts run AFTER voters — cleans up precinct_voters as its last step
            $precinctMsg = archivePrecincts($pdo, $archivePdo, $election_id, $voting_period_id);
            preg_match('/\d+/', $precinctMsg, $matches);
            $archivedCounts['precincts'] = (int)($matches[0] ?? 0);

            // ── Insert / update archived_details_short ────────────────────
            ...
            // ── Update statuses for this election cycle ───────────────────
            $pdo->prepare("UPDATE voting_periods     SET status='Published' WHERE id=?")->execute([$voting_period_id]);
            ...
            $pdo->prepare("UPDATE events             SET status='Ended'     WHERE candidacy=?")->execute([$election_id]);
        }
    }
}

// ============================================================
// STEP 2 — Create the new academic year and archive the old one
// ============================================================
try {
    $pdo->beginTransaction();

    // Mark all currently Ongoing academic years as Archived
    $pdo->exec("UPDATE academic_years SET status = 'Archived' WHERE status = 'Ongoing'");

    // Insert the new academic year
    $stmt = $pdo->prepare("
        INSERT INTO academic_years
            (year_label, semester, start_date, end_date, status, custom_voter_option)
        VALUES
            (?, ?, ?, ?, 'Ongoing', ?)
    ");
    $stmt->execute([$school_year, $semester, $start_date, $end_date, $custom_voter_option]);

    // Deactivate all existing voters so they re-register under the new year
    $pdo->exec("UPDATE voters SET is_active = 0, needs_update = 1, status = 'archived'");

    $pdo->commit();

    $_SESSION['status']  = 'success';
    $_SESSION['message'] = 'New academic year created and previous records archived.';
    header("Location: ../../academic_info.php");
    exit;
...
/**
 * Fetch all reference/lookup data for an academic year from the main DB.
 */
function fetchAcademicYearData($pdo, $academic_year_id)
{
    ...
    $stmt = $pdo->prepare("
        SELECT ay.id AS academic_year_id, ay.year_label, ay.semester,
               ay.start_date AS ay_start, ay.end_date AS ay_end,
               ay.status AS ay_status, ay.custom_voter_option
        FROM academic_years ay WHERE ay.id = ? LIMIT 1
    ");
    $stmt->execute([$academic_year_id]);
    $ayData['academic_year'] = $stmt->fetch(PDO::FETCH_ASSOC);

    $ayData['campuses']    = $pdo->query("SELECT * FROM campuses")->fetchAll(PDO::FETCH_ASSOC);
    $ayData['departments'] = $pdo->query("SELECT * FROM departments")->fetchAll(PDO::FETCH_ASSOC);
    $ayData['year_levels'] = $pdo->query("SELECT * FROM year_levels")->fetchAll(PDO::FETCH_ASSOC);

    $stmt = $pdo->query("
        SELECT c.*, cc.coordinate_id,
               cc.campus_id  AS cc_campus_id,
               cc.latitude   AS cc_latitude,
               cc.longitude  AS cc_longitude
        FROM colleges c
        LEFT JOIN college_coordinates cc ON cc.college_id = c.college_id
    ");
    $ayData['colleges'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $ayData['actual_year_levels'] = $pdo->query("SELECT * FROM actual_year_levels")->fetchAll(PDO::FETCH_ASSOC);

    $courses = $pdo->query("SELECT * FROM courses")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($courses as &$course) {
        $s = $pdo->prepare("SELECT * FROM course_year_levels WHERE course_id = ?");
        $s->execute([$course['id']]);
        $course['year_levels'] = $s->fetchAll(PDO::FETCH_ASSOC);
    }
    $ayData['courses'] = $courses;

    $majors = $pdo->query("SELECT * FROM majors")->fetchAll(PDO::FETCH_ASSOC);
    foreach ($majors as &$major) {
        $s = $pdo->prepare("SELECT * FROM major_year_levels WHERE major_id = ?");
        $s->execute([$major['major_id']]);
        $major['year_levels'] = $s->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    if ($ayData['academic_year']['custom_voter_option'] == 1) {
        ...
        if (!empty($fieldIds)) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM voter_custom_files WHERE field_id IN ($ph)");
            $stmt->execute($fieldIds);
            $ayData['voter_custom_files'] = $stmt->fetchAll(PDO::FETCH_ASSOC);

            $stmt = $pdo->prepare("SELECT * FROM voter_custom_responses WHERE field_id IN ($ph)");
            $stmt->execute($fieldIds);
...
/**
 * Archive a single academic year by ID.
 * BUG FIX (Bug 4): Now scoped to a specific academic_year_id instead of
 * archiving every non-archived academic year in the system.
 */
function archiveAcademicYear($pdo, $archivePdo, $academic_year_id)
{
    try {
        ...
        if (!$rows) return "No academic years to archive (already archived or not found).";
...
/**
 * Archive all reference/lookup tables.
 */
function archiveAcademicInfo($archivePdo, $ayData)
{
    ...
    foreach ($ayData['actual_year_levels'] as $ayl) {
        $archivePdo->prepare("
            INSERT INTO archived_actual_year_levels (id, course_id, major_id, year_level, archived_on)
            VALUES (?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                ...
                archived_on = VALUES(archived_on)
        ")->execute([$ayl['id'], $ayl['course_id'], $ayl['major_id'], $ayl['year_level'], $now]);
    }
    ...
    // colleges + coordinates
    foreach ($ayData['colleges'] as $college) {
        $archivePdo->prepare("
            INSERT INTO archived_colleges (college_id, college_name, college_abbreviation)
            VALUES (?, ?, ?)
            ON DUPLICATE KEY UPDATE
                college_name         = VALUES(college_name),
                college_abbreviation = VALUES(college_abbreviation)
        ")->execute([$college['college_id'], $college['college_name'], $college['college_abbreviation']]);
    ...
    // courses + course year levels
    foreach ($ayData['courses'] as $course) {
        $archivePdo->prepare("
            INSERT INTO archived_courses (id, college_id, course_name, course_code)
            VALUES (?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                college_id  = VALUES(college_id),
                course_name = VALUES(course_name),
                course_code = VALUES(course_code)
        ")->execute([$course['id'], $course['college_id'], $course['course_name'], $course['course_code']]);

        foreach ($course['year_levels'] as $cyl) {
            $archivePdo->prepare("
                INSERT INTO archived_course_year_levels (id, course_id, year_level_id)
                VALUES (?, ?, ?)
                ON DUPLICATE KEY UPDATE
                    course_id     = VALUES(course_id),
                    year_level_id = VALUES(year_level_id)
            ")->execute([$cyl['id'], $cyl['course_id'], $cyl['year_level_id']]);
        }
    }
    ...
    foreach ($ayData['departments'] as $dept) {
        $archivePdo->prepare("
            INSERT INTO archived_departments (department_id, department_name, college_id)
            VALUES (?, ?, ?)
            ON DUPLICATE KEY UPDATE
                department_name = VALUES(department_name),
                college_id      = VALUES(college_id)
        ")->execute([$dept['department_id'], $dept['department_name'], $dept['college_id']]);
    }

    // majors + major year levels
    foreach ($ayData['majors'] as $major) {
        $archivePdo->prepare("
            INSERT INTO archived_majors (major_id, major_name, course_id)
            VALUES (?, ?, ?)
            ON DUPLICATE KEY UPDATE
                major_name = VALUES(major_name),
                course_id  = VALUES(course_id)
        ")->execute([$major['major_id'], $major['major_name'], $major['course_id']]);

        foreach ($major['year_levels'] as $myl) {
            $archivePdo->prepare("
                INSERT INTO archived_major_year_levels (id, major_id, year_level_id)
                VALUES (?, ?, ?)
                ON DUPLICATE KEY UPDATE
                    major_id      = VALUES(major_id),
                    year_level_id = VALUES(year_level_id)
            ")->execute([$myl['id'], $myl['major_id'], $myl['year_level_id']]);
        }
    }
    ...
    foreach ($ayData['year_levels'] as $yl) {
        $archivePdo->prepare("
            INSERT INTO archived_year_levels (id, level, description)
            VALUES (?, ?, ?)
            ON DUPLICATE KEY UPDATE
                level       = VALUES(level),
                description = VALUES(description)
        ")->execute([$yl['id'], $yl['level'], $yl['description']]);
    }

    // custom voter data (only if enabled)
    if (($ayData['academic_year']['custom_voter_option'] ?? 0) == 1) {

        foreach ($ayData['voter_columns'] as $vc) {
            $archivePdo->prepare("
                INSERT INTO archived_voters_columns (id, academic_year_id, number)
                VALUES (?, ?, ?)
                ON DUPLICATE KEY UPDATE number = VALUES(number)
            ")->execute([$vc['id'], $vc['academic_year_id'], $vc['number']]);
        }
        ...
        foreach ($ayData['voter_custom_files'] as $vcf) {
            $archivePdo->prepare("
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    archived_on = VALUES(archived_on)
            ")->execute([$vcf['id'], $vcf['voter_id'], $vcf['field_id'], $vcf['file_path'], $vcf['uploaded_at'], $now]);
        }
        ...
        foreach ($ayData['voter_custom_responses'] as $vcr) {
            $archivePdo->prepare("
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    archived_on = VALUES(archived_on)
            ")->execute([$vcr['id'], $vcr['voter_id'], $vcr['field_id'], $vcr['field_value'], $vcr['created_at'], $now]);
        }
    }
}
...
{
    $stmt = $pdo->prepare("
        SELECT e.id, e.academic_year_id, e.election_name,
...
function archivePositions($pdo, $archivePdo, $election_id)
{
    ...
    foreach ($positions as $pos) {
        $archivePdo->prepare("
            INSERT INTO archived_positions (id, name, party, level, election_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                name        = VALUES(name),
                party       = VALUES(party),
                ...
                election_id = VALUES(election_id),
                created_at  = VALUES(created_at)
        ")->execute([$pos['id'], $pos['name'], $pos['party'], $pos['level'], $pos['election_id'], $pos['created_at']]);
    }
...
/**
 * Archive candidates with proper winner determination per position+level+college group.
 */
function archiveCandidates($pdo, $archivePdo, $election_id, $voting_period_id)
{
    ...
    // Pass 1: resolve metadata & vote counts
    $processedCandidates = [];
    $groupMaxVotes       = [];

    foreach ($candidates as $cand) {
        $sl = $pdo->prepare("SELECT level FROM positions WHERE name = ? AND election_id = ? LIMIT 1");
        $sl->execute([$cand['position'], $election_id]);
        $level = $sl->fetchColumn() ?: 'Local';

        $sc = $pdo->prepare("SELECT college FROM voters WHERE student_id = ? LIMIT 1");
        $sc->execute([$cand['student_id']]);
        $college = $sc->fetchColumn();

        $sp = $pdo->prepare("SELECT file_path FROM candidate_files WHERE candidate_id = ? LIMIT 1");
        $sp->execute([$cand['id']]);
        $picPath      = $sp->fetchColumn();
        $picture_path = $picPath ? basename($picPath) : '';

        $sv = $pdo->prepare("
            SELECT
                COUNT(DISTINCT v.id)                                                    AS total,
                SUM(CASE WHEN cam.campus_name = 'Main Campus' THEN 1 ELSE 0 END)       AS internal,
                SUM(CASE WHEN cam.campus_name != 'Main Campus'
                          OR cam.campus_name IS NULL THEN 1 ELSE 0 END)                 AS external
            FROM votes v
            LEFT JOIN precinct_voters pv  ON v.student_id   = pv.student_id
            LEFT JOIN precincts       p   ON pv.precinct    = CAST(p.id AS CHAR)
            LEFT JOIN campuses        cam ON p.type         = cam.campus_id
            WHERE v.candidate_id     = ?
              AND v.voting_period_id = ?
        ");
        $sv->execute([$cand['id'], $voting_period_id]);
        $voteStats = $sv->fetch(PDO::FETCH_ASSOC);

        $cand['level']        = $level;
        $cand['college']      = $college;
        $cand['picture_path'] = $picture_path;
        $cand['votes']        = (int)($voteStats['total']    ?? 0);
        $cand['internal']     = (int)($voteStats['internal'] ?? 0);
        $cand['external']     = (int)($voteStats['external'] ?? 0);

        $groupKey  = $level . '|' . $cand['position'];
        $groupKey .= ($level === 'Central') ? '|All' : '|' . $college;

        if (!isset($groupMaxVotes[$groupKey]) || $cand['votes'] > $groupMaxVotes[$groupKey]) {
    ...
    // Pass 2: insert with correct Won/Lost outcome
    foreach ($processedCandidates as $cand) {
        $groupKey  = $cand['level'] . '|' . $cand['position'];
        $groupKey .= ($cand['level'] === 'Central') ? '|All' : '|' . $cand['college'];
...
function archiveCandidacyData($pdo, $archivePdo, $election_id, $voting_period_id)
{
    try {
        ...
        if (!$candidacy) return "No candidacy found for election ID: $election_id";

        $sc = $pdo->prepare("
            SELECT COUNT(*)
            FROM candidates ca
            JOIN registration_forms rf ON ca.form_id = rf.id
            WHERE rf.election_name = ? AND ca.status = 'accepted'
        ");
        $sc->execute([$election_id]);
        $totalFiled = (int)$sc->fetchColumn();
    ...
    foreach ($parties as $party) {
        $archivePdo->prepare("
            INSERT INTO archived_parties
                (id, name, election_id, party_image, platforms, created_at, updated_at,
                 status, archived_on, voting_period_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
                name        = VALUES(name),
...
/**
 * Archive voters.
 * BUG FIX (Bug 1 & 3): Must be called BEFORE archivePrecincts.
 * Removed OR pv.student_id IS NULL — that pulled in voters from all elections.
 * Uses INNER JOINs throughout to strictly scope to this election only.
 */
...
/**
 * Archive precincts and clean up live precinct_voters rows.
 * BUG FIX (Bug 1): Must run AFTER archiveVoters — the DELETE at the end
 * removes precinct_voters rows that archiveVoters depends on.
 */
