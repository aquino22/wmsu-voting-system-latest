...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        ...
        // Publish the event (combine both updates)
        $sqlPublish = "
            UPDATE events 
        ...
        $stmtPublish->bindParam(':id', $event_id, PDO::PARAM_INT);
        ...
        $publishSuccess = $stmtPublish->execute();
        $formsSuccess = $stmtForms->execute();

        if ($publishSuccess && $formsSuccess) {
            $affectedForms = $stmtForms->rowCount();
            $pdo->commit();
            $_SESSION['STATUS'] = "PUBLISH_SUCCESSFUL";
            $_SESSION['MESSAGE'] = "Event published successfully. $affectedForms related form(s) updated.";
        } else {
            $pdo->rollBack();
            throw new Exception(
                !$publishSuccess ? "Failed to publish event." : 
        ...
        $pdo->rollBack();
        $_SESSION['STATUS'] = "PUBLISH_UNSUCCESSFUL";
