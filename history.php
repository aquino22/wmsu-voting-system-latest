...
<body>
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
      ...
      function getElectionHistory($pdo)
      {
        $stmt = $pdo->prepare("
        SELECT 
            id,
    ...
    ");
      ...
      function getCandidacyHistory($pdo)
      {
        $stmt = $pdo->prepare("
        SELECT 
            id,
    ...
    ");
      ...
      function getCandidatesForElection($pdo, $voting_period_id)
      {
        ...
        LEFT JOIN archived_voters av ON ac.voting_period_id = av.voting_period_id 
        AND (
    ...
    ");
      ...
      $electionHistory = getElectionHistory($pdo);
      $candidacyHistory = getCandidacyHistory($pdo);

      // 2. Fetch all archived voters
      $stmt = $pdo->prepare("SELECT * FROM archived_voters WHERE status = 'archived' ORDER BY id DESC");
      $stmt->execute();
      ...
      $stmt = $archived_pdo->prepare("SELECT * FROM archived_precincts ORDER BY archived_at DESC");
      $stmt->execute();
       ...
       */
      function getArchivedPositions($pdo)
      {
        $stmt = $pdo->prepare("
        SELECT ap.*, ae.election_name
        FROM archived_positions ap
        LEFT JOIN archived_elections ae ON ap.election_id = ae.id
    ...
    ");
       ...
       */
      function getArchivedParties($pdo)
      {
        $stmt = $pdo->prepare("
        SELECT *
        FROM archived_parties
        ORDER BY election_name DESC, name ASC
    ");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
      }
      ...
      $archivedPositions = getArchivedPositions($pdo);
      $archivedParties = getArchivedParties($pdo);
                              ...
                              <tbody>
                                <?php foreach ($electionHistory as $election): ?>
                                  <tr>
                                    ...
                                    <td><?= $election['school_year_start'] . '-' . $election['school_year_end']; ?></td>
                                    <td><?php echo htmlspecialchars($election['semester']); ?></td>
                                    <td><?php echo htmlspecialchars($election['election_name']); ?></td>
                                    <td><?php echo htmlspecialchars($election['start_period']); ?></td>
                                    <td><?php echo htmlspecialchars($election['end_period']); ?></td>
                                    <td>
                            ...
                            <div class="row mt-3">
                              ...
                              <div class="col-md-6">
                                ...
                                <select id="voterSYFilter" class="form-select">
                                  ...
                                  $uniqueSY = [];
                                  foreach ($electionHistory as $e) {
                                    $syStr = $e['school_year_start'] . '-' . $e['school_year_end'] . ' ' . $e['semester'];
                            ...
                            <table id="archivedVotersTable" class="table table-striped table-bordered nowrap" style="width:100%">
                              ...
                              <tbody>
                                <?php foreach ($voters as $voter):
                                  ...
                                  $sy = $election ? $election['school_year_start'] . '-' . $election['school_year_end'] : 'N/A';
                                  $sem = $election ? $election['semester'] : 'N/A';
                                  $sy_sem = $sy . ' ' . $sem;
                                ?>
                                  <tr>
                                    <td><?= htmlspecialchars($sy_sem) ?></td>
                    ...
                    <div class="card card-rounded mb-5">
                      <div class="card-body">
                        <div class="row mt-4">
                          ...
                          <div class="table-responsive">
                            <?php
                            // First get all unique elections from archived precincts
                            $elections_query = "SELECT DISTINCT election FROM archived_precincts ORDER BY archived_at DESC";
                            ...
                            if (count($elections) > 0) {
                              foreach ($elections as $election) {
                                ...
                                echo "<h4 class='mt-1 mb-3'><b>Archived Election Name:</b> " . htmlspecialchars($election_name) . "</h4>";

                                // Get precincts for this specific election
                                $query = "SELECT * FROM archived_precincts 
                                 WHERE election = :election_name
                                 ORDER BY id DESC";
                                ...
                                <tbody>';
                                  ...
                                  foreach ($archived_precincts as $row) {
                                    ...
                                    if ($row['type'] === 'External' && !empty($row['college_external'])) {
                                      echo " (" . htmlspecialchars($row['college_external']) . ")";
                                    }
                              ...
                              <tbody>
                                <?php foreach ($candidacyHistory as $candidacy): ?>
                                  <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($candidacy['semester'] . ', ' . $candidacy['school_year_start'] . '-' . $candidacy['school_year_end']); ?></td>
        ...
        <?php foreach ($electionHistory as $election): ?>
          ...
          <div class="modal fade" id="viewWinnersModal<?php echo $election['id']; ?>" tabindex="-1" aria-labelledby="viewWinnersModalLabel<?php echo $election['id']; ?>" aria-hidden="true">
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="viewWinnersModalLabel<?php echo $election['id']; ?>">Winners - <?php echo htmlspecialchars($election['election_name']); ?> (<?php echo htmlspecialchars($election['semester'] . ', ' . $election['school_year_start'] . '-' . $election['school_year_end']); ?>)</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <dl class="row">
                    ...
                    <dd class="col-sm-8"><?php echo htmlspecialchars($election['semester'] . ', ' . $election['school_year_start'] . '-' . $election['school_year_end']); ?></dd>
        ...
        <?php foreach ($candidacyHistory as $candidacy): ?>
          ...
          <div class="modal fade" id="viewCandidatesModal<?php echo $candidacy['id']; ?>" tabindex="-1" aria-labelledby="viewCandidatesModalLabel<?php echo $candidacy['id']; ?>" aria-hidden="true">
            <div class="modal-dialog modal-lg">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="viewCandidatesModalLabel<?php echo $candidacy['id']; ?>">Candidates - <?php echo htmlspecialchars($candidacy['election_name']); ?> (<?php echo htmlspecialchars($candidacy['semester'] . ', ' . $candidacy['school_year_start'] . '-' . $candidacy['school_year_end']); ?>)</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <div class="mb-3">
                    <p><strong>School Year:</strong> <?php echo htmlspecialchars($candidacy['school_year_start'] . '-' . $candidacy['school_year_end']); ?></p>
                    <p><strong>Semester:</strong> <?php echo htmlspecialchars($candidacy['semester']); ?></p>
                          ...
                          <tbody>
                            <?php foreach ($positionCandidates as $candidate): ?>
                              <tr>
                                ...
                                <td><?php echo htmlspecialchars($candidate['first_name'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($candidate['middle_name'] ?? ''); ?></td>
                                <td><?php echo htmlspecialchars($candidate['last_name'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($candidate['course'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($candidate['college'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($candidate['department'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($candidate['year_level'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars(($candidate['wmsu_campus'] ?? '') . ($candidate['external_campus'] ? ' - ' . $candidate['external_campus'] : '')); ?></td>
                                <td><?php echo htmlspecialchars($candidate['party'] ?? 'N/A'); ?></td>
                                <td><?php echo htmlspecialchars($candidate['outcome']); ?></td>
                              </tr>
                            <?php endforeach; ?>
                          </tbody>
                        </table>
      ...
      <script>
        $(document).ready(function() {
          ...
          $('#voterElectionFilter').on('change', function() {
            ...
            votersTable.column(10).search(val ? '^' + val + '$' : '', true, false).draw();
          });

          $('#voterSYFilter').on('change', function() {
            var val = $.fn.dataTable.util.escapeRegex($(this).val());
            votersTable.column(0).search(val ? '^' + val + '$' : '', true, false).draw();
          });
