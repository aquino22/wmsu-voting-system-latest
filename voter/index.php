...
// FIX: Use LEFT JOIN so we always get the precinct_voters row even when the
//      precincts.id/pv.precinct types don't match perfectly. $isInPrecinct is
//      determined by whether the student exists in precinct_voters — not by
//      whether the JOIN to precincts succeeded.
$status            = null;
$college           = null;
$precinct_name     = null;
$voter_precinct_id = null;   // FIX: stored once here, never re-queried later
$collegeName       = 'N/A';
...
if ($student_id) {
    try {
        $stmt = $pdo->prepare("
            ...
            LEFT JOIN precincts p ON p.id = pv.precinct   -- LEFT so row survives type mismatch
            WHERE pv.student_id = ?
            LIMIT 1
        ");
        ...
        if ($voter) {
            $voter_precinct_id = $voter['precinct'];          // FIX: capture precinct ID
            $precinct_name     = $voter['precinct_name'];     // may be null if JOIN missed
            $status            = $voter['status'];
...
// FIX: $isInPrecinct is based purely on existence in precinct_voters,
//      not on whether the JOIN to precincts succeeded.
$isInPrecinct = ($voter_precinct_id !== null);
...
try {
    $stmt = $pdo->prepare("
        ...
        LEFT JOIN votes     v  ON vp.id = v.voting_period_id AND v.student_id = ?
...
// FIX: Single JOIN query using the already-known $student_id and $voter_precinct_id.
//      No LIMIT 1 re-query, no PHP-side int/varchar casting issue.
$precinctsAssignedToElection = [];
foreach ($votingPeriods as $period) {
    $electionId = $period['election_id'];
    if (isset($precinctsAssignedToElection[$electionId])) continue;

    if (!$isInPrecinct) {
        $precinctsAssignedToElection[$electionId] = false;
    } else {
        // FIX: Let the DB do the join — avoids the PHP varchar/int mismatch entirely.
        $stmt = $pdo->prepare("
            SELECT COUNT(*)
            FROM precinct_voters  pv
            JOIN precinct_elections pe ON pe.precinct_id = pv.precinct
            WHERE pv.student_id    = ?
              AND pe.election_name = ?
        ");
        $stmt->execute([$student_id, $electionId]);
...
if ($selectedElection) {
    ...
    // Countdown seconds
    $useReSchedule = !empty($votingPeriodReStart) && !empty($votingPeriodReEnd);
    ...
    // Only load candidates if voter status allows it
...
<body>
    <div class="container-scroller">
        ...
        <div class="container-fluid page-body-wrapper">
            ...
            <nav class="sidebar sidebar-offcanvas" id="sidebar">
                <ul class="nav">
                    <li class="nav-item">
                        <a class="nav-link" href="index.php" style="background-color:#B22222 !important;">
    ...
    <?php if (isset($_SESSION['STATUS']) && $_SESSION['STATUS'] === 'NOT_VOTER'): ?>
        <script>
            Swal.fire({
                icon: 'error',
                title: 'Access Denied',
                text: 'You are not assigned to a precinct for this election.'
            });
        </script>
