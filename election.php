                      ...
                      <div class="card card-rounded mb-5">
                        <div class="card-body">
                          <div class="d-flex align-items-center mb-3">
                            <h3 class="mb-0"><b>Election Periods</b></h3>
                            <div class="ms-auto d-flex">
                              <button class="btn btn-primary text-white" data-bs-toggle="modal"
                                data-bs-target="#electionModal">
                                <small><i class="mdi mdi-plus-circle"></i> Set New Election Period</small>
                              </button>
                              <?php

                              // Query to get distinct semesters and school year ranges from the elections table
...
ORDER BY semester DESC";
                              ...
                              <tbody>
                                ...
                                try {
                                  ...
                                  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $electionEndPeriod = new DateTime($row['end_period']);
                                    if ($currentDateTime > $electionEndPeriod && $row['status'] !== 'Ended') {
                                      $updateQuery = "UPDATE elections SET status = 'Ended' WHERE id = :id";
                                    ...
                                    $startPeriod = !empty($row['start_period']) ? (new DateTime($row['start_period']))->format('M j, Y \a\t g:i A') : 'N/A';
                                    $endPeriod = !empty($row['end_period']) ? (new DateTime($row['end_period']))->format('M j, Y \a\t g:i A') : 'N/A';

                                    $badgeClass = $row['status'] == 'Ongoing' ? 'bg-success' : ($row['status'] == 'Upcoming' ? 'bg-warning' : 'bg-danger');

                                    $startPeriodRaw = !empty($row['start_period']) ? (new DateTime($row['start_period']))->format('Y-m-d\TH:i') : '';
                                    $endPeriodRaw = !empty($row['end_period']) ? (new DateTime($row['end_period']))->format('Y-m-d\TH:i') : '';

                                    echo "<tr>
            <td>" . htmlspecialchars($row['election_name']) . "</td>
            <td>" . htmlspecialchars($row['semester']) . "</td>
            <td>" . htmlspecialchars($row['school_year_start']) . "</td>
            <td>" . htmlspecialchars($row['school_year_end']) . "</td>
            <td>" . htmlspecialchars($startPeriod) . "</td>
            <td>" . htmlspecialchars($endPeriod) . "</td>
            <td><span class='badge $badgeClass'>" . htmlspecialchars($row['status']) . "</span></td>
            <td>";

                                    // Action buttons logic
                                    if ($row['status'] === 'Upcoming' || $row['status'] === 'Ongoing') {
                                      echo "
                <button class='button-view-warning edit-election' 
                    data-bs-toggle='modal' 
                    data-bs-target='#editElectionModal'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-election-name='" . htmlspecialchars($row['election_name']) . "'
                    data-school-year-start='" . htmlspecialchars($row['school_year_start']) . "'
                    data-school-year-end='" . htmlspecialchars($row['school_year_end']) . "'
                    data-semester='" . htmlspecialchars($row['semester']) . "'
                    data-start-period='" . htmlspecialchars($startPeriodRaw) . "'
                    data-end-period='" . htmlspecialchars($endPeriodRaw) . "'
                    data-status='" . htmlspecialchars($row['status']) . "'>
                    <i class='mdi mdi-pencil'></i> Edit
                </button>

                <button class='button-view-danger delete-election' 
                    data-id='" . htmlspecialchars($row['id']) . "' 
            ...
            ";
                                      ...
                                      echo "
                <a href='history.php?id=" . urlencode($row['id']) . "' 
                   class='button-view-primary'>
                    <i class='mdi mdi-poll'></i> View Results
                </a>
            ";
                                    } else {
                                      // Optional: Ended or other status can just show nothing or a View Results
                                      echo "
                <a href='history.php?id=" . urlencode($row['id']) . "' 
                   class='button-view-primary'>
                    <i class='mdi mdi-poll'></i> View Results
                      ...
                      <div class="row mt-4">
                        <div class="container  mb-5">
                          <div class="d-flex align-items-center">
                            <h3><b>Party </b></h3>
                            <div class="ms-auto">
                              <small> <button class="btn btn-primary text-white" data-bs-toggle="modal" data-bs-target="#partyModal">
                        ...
                        <?php
                        // Fetch the party data from the database
                        try {
                          $stmt = $pdo->prepare("SELECT * FROM parties");
                          $stmt->execute();
                            ...
                            <?php if (!empty($parties)): ?>
                              <?php foreach ($parties as $party): ?>
                                <tr>
                                  ...
                                  <td><?php echo htmlspecialchars($party['election_name']); ?></td>
                                  <td>
                                    <?php if (!empty($party['platforms'])): ?>
                                      <button class="btn  btn-sm viewPlatformsBtn"
                                        data-id="<?php echo $party['id']; ?>"
                                        data-platforms="<?php echo htmlspecialchars($party['platforms'], ENT_QUOTES, 'UTF-8'); ?>"
                                        data-bs-toggle="modal" data-bs-target="#viewPlatformsModal">
                                  ...
                                  <td>
                                    <?php
                                    $status = strtolower($party['status']); // make case-insensitive
                                    $badgeClass = '';
                                    ...
                                    switch ($status) {
                                      ...
                                      case 'disapproved':
                                        ...
                                        break;
                                      case 'unverified':
                                        $badgeClass = 'bg-secondary text-black'; // Gray
                                        break;
                                      case 'published':
                                        $badgeClass = 'bg-primary'; // Blue
                                        break;
                                      default:
                                        $badgeClass = 'bg-warning text-black'; // Yellow for unknown
                                        break;
                                    }
                                    ?>
                                  ...
                                  <td>
                                    <?php if (strtolower($party['status']) === 'published'): ?>
                      ...
                      <div class="row mt-4">
                        <div class="container  mb-5">
                          <div class="d-flex align-items-center">
                            <h3><b>Position </b></h3>
                            <div class="ms-auto">
                              <small><button class="btn btn-primary text-white" data-bs-toggle="modal"
            ...
            WHERE id = :id");
                  ...
                  <div class="modal fade" id="partyModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <form id="partyForm" enctype="multipart/form-data" method="POST" action="processes/elect/add_party.php">
                            <?php
                            ...
                            $query = "SELECT election_name FROM elections WHERE status IN ('Ongoing')";
                            $stmt = $pdo->query($query);
                            ...
                            <div class="mb-3">
                              ...
                              <select class="form-select" id="election" name="election" required>
                                ...
                                <?php foreach ($elections as $election): ?>
                                  <option value="<?= htmlspecialchars($election['election_name']); ?>">
                  ...
                  <div class="modal fade" id="electionModal" tabindex="-1" aria-labelledby="exampleModalLabel"
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h1 class="modal-title fs-5" id="exampleModalLabel">Add a New Election</h1>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                          <form id="electionForm">
                            ...
                            <div class="mb-3">
                              <label for="endPeriod" class="form-label">
                                End Period: <small>(Usually 1 month and two weeks from now)</small>
                              </label>
                            ...
                            <script>
                              ...
                              // Optional: ensure endPeriod is always after startPeriod
                              startInput.addEventListener('change', () => {
                                const startDate = new Date(startInput.value);
                                if (!isNaN(startDate)) {
                                  endInput.min = formatDateTime(startDate);
                                }
                              });
                            </script>
    ...
    `;
                  ...
                  document.addEventListener('DOMContentLoaded', function() {
                    function populateModal(button) {
                      try {
                        ...
                        const schoolYearStart = button.getAttribute('data-school-year-start');
                        const schoolYearEnd = button.getAttribute('data-school-year-end');
                        ...
                        document.getElementById('editSchoolYearStart').value = schoolYearStart || '';
                        document.getElementById('editSchoolYearEnd').value = schoolYearEnd || '';
                      ...
                      <div class='modal-body'>
                        <form id="editElectionForm">
                          ...
                          <div class="mb-3">
                            ...
                            <div class="d-flex gap-2">
                              <input type="number" class="form-control" name="school_year_start" id="editSchoolYearStart" required>
                              <input type="number" class="form-control" name="school_year_end" id="editSchoolYearEnd" required>
                            </div>
                          </div>
                          <div class="mb-3">
                          ...
                          <div class="mb-3">
                            <label for="endPeriod" class="form-label">End Period: <small>(Usually 1 month and two weeks from now)</label>
                ...
                <script>
                  document.addEventListener('DOMContentLoaded', function() {
                    ...
                    levelSelect.addEventListener('change', function() {
                      ...
                      let positions = [];
                      if (level === 'Central') {
                        positions = centralPositions;
                      } else if (level === 'Local') {
                        positions = localPositions;
                      }

                      renderDropdown(positions);
      ...
      `;
                    ...
                    // Trigger change initially if needed
