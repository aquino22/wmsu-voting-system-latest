			...
			<?php if (!empty($customFields)): ?>
				...
				<?php foreach ($customFields as $field): ?>
					...
					<div class="col-md-<?= $bsColWidth ?>" <?= $wrapperId ? 'id="' . $wrapperId . '"' : '' ?>>
						<label class="text-white"><?= htmlspecialchars($label) ?>:</label>
						...
						<?php elseif ($inputType === 'file'): ?>
							...
							<div class="wrap-input100 <?= $validateClass ?> custom-file mt-2" data-validate="Upload <?= htmlspecialchars($label) ?>">
								<input class="custom-file-input" type="file" name="<?= $name ?>" id="<?= $fileId ?>" <?= $isRequired ?> <?= $accept ?> onchange="handleFileSelect(this)">
								...
								<a id="previewBtn_<?= $fileId ?>"
									...
									style=" display:none; cursor:pointer; text-decoration: underline; color: #fff;"
									onclick="previewFile(this, '<?= $fileId ?>')">
									Preview File
								</a>
...
<script>
	function handleFileSelect(input) {
		...
		const previewBtn = document.getElementById('previewBtn_' + input.id);

		if (file) {
			previewBtn.style.display = 'inline-block'; // show button
		} else {
			previewBtn.style.display = 'none'; // hide button if no file
		}

		// store file in button dataset for preview
		previewBtn.dataset.fileId = input.id;
	}

	function previewFile(button, fileInputId) {
		...
		reader.onload = function(e) {
			...
			if (type.startsWith("image/")) {
				...
				container.innerHTML = `
                <p>Preview not available. File ready for upload.</p>
